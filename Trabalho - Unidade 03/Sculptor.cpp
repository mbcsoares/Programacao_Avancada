...
{
	if ((_x >= 0 && _x <= nx - 1) && (_y >= 0 && _y <= ny - 1) && (_z >= 0 && _z <= nz - 1))
	{
...
void Sculptor::putBox(int x0, int x1, int y0, int y1, int z0, int z1)
{
	...
	/*if (  ((x1 - x0 + 1) <= nx) && ((y1 - y0 + 1) <= ny) && ((z1 - z0 + 1) <= nz)  )
	{
		for (x = x0; x <= x1; x++)
		{
			for (y = y0; y <= y1; y++)
...
void Sculptor::putSphere(int xCenter, int yCenter, int zCenter, int radius)
{
	...
	/*if (  (xCenter + radius) <= (nx - 1) && (yCenter + radius) <= (ny - 1) && (zCenter + radius) <= (nz - 1)  )
	{
		if (  (xCenter - radius) >= 0 && (yCenter - radius) >= 0 && (zCenter - radius) >= 0  )
		{
			for (x = xCenter - radius; x <= xCenter + radius; x++)
			{
				yDelta = sqrt(pow(radius, 2) - pow(x - xCenter, 2));
...
void Sculptor::putEllipsoid(int xCenter, int yCenter, int zCenter, int rx, int ry, int rz)
{
	...
	/*if ((xCenter + rx) <= (nx - 1) && (yCenter + ry) <= (ny - 1) && (zCenter + rz) <= (nz - 1))
	{
		if ((xCenter - rx) >= 0 && (yCenter - ry) >= 0 && (zCenter - rz) >= 0)
		{
			for (x = xCenter - rx; x <= xCenter + rx; x++)
			{
				yDelta = ry * sqrt(1.0 - pow(1.0 * (x - xCenter) / rx, 2));
